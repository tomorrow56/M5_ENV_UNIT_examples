...
#include "UNIT_ENV.h"   // https://github.com/m5stack/UNIT_ENV
...
void task0(void* arg) {
  while (1) {
    ...
    if(sendFlag == false){
      ...
      M5.Lcd.printf("Pressure   : %4.0f hPa  ", pressure / 100);
...
void setup() {
  ...
  while (!qmp.init()){  
    Serial.println("Could not find a valid QMP6988 sensor, check wiring!");
